...
export default {
  ...
  methods: {
    ...
    changeFile(event) {
      ...
      if (event.target.files.length) {
        if (this.allowed.includes(event.target.files[0].type)) {
          input.value.file = event.target.files[0];
          input.value.preview = URL.createObjectURL(event.target.files[0]);
    ...
    drop(event) {
      ...
      if (event.dataTransfer.files.length) {
        if (this.allowed.includes(event.dataTransfer.files[0].type)) {
          this.input.file = event.dataTransfer.files[0];
          this.input.preview = URL.createObjectURL(event.dataTransfer.files[0]);
        } else {
          this.$notify({
            title: this.$t('Unsupported file type').toString(),
            text: this.$t('Only image type files are accepted').toString(),
