<template>
  <main class="flex-1 relative overflow-y-auto focus:outline-none" tabindex="0">
    <div class="my-6 max-w-1/6 mx-auto px-4 sm:px-6 lg:px-8">
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex min-w-0">
          <svg-vue class="content-title-icon" :icon="['fas', 'table-list']"></svg-vue>
          ...
          <div class="py-2">
            <div class="flex items-center justify-between">
              <div tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl leading-normal text-gray-800">
                <input type="text" id="search" v-model="filters.search" :placeholder="$t('Search')" @change="getProducts" class="form-input" />
              </div>
              <div class="flex items-center space-x-3">
                ...
                <div class="flex border-2 border-gray-300 rounded-md">
                  <select @change="changeSort" aria-label="Sort by" class="form-input border-none" id="status" v-model="sort.column">
                    ...
                    <option value="category_id">{{ $t('Category') }}</option>
                    <option value="created_at">{{ $t('Created at') }}</option>
                    <option value="updated_at">{{ $t('Updated at') }}</option>
                  </select>
                  <button @click="changeSort" class="flex text-gray-800 font-bold px-2 form-input border-none" type="button">
            ...
            <div class="bg-gray-100 p-2 mb-2 border sm:flex items-center justify-between">
              <div class="flex md:space-x-2">
                <template v-if="isThereSelectedRow">
                  <button class="btn btn-red" @click.prevent="productDeleteModel = true">{{ $t('Delete') }}</button>
                </template>
                <template v-else>
                  <router-link to="/admin/products/new" class="btn btn-app">
                    {{ $t('Add') }}
                  </router-link>
                </template>
              </div>
              <select @change="getProducts" class="form-input py-3 w-1/4" id="category" v-model="filters.category">
                <option :value="null">{{ $t('By category') }}</option>
                <option v-for="(category, cindex) in productCategories" :key="cindex" :value="category.id">{{ category.name }}</option>
              </select>
            </div>
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="flex py-2 items-center">
                        <img :alt="$t('Avatar')" :src="product.image" class="h-12 w-12 rounded" />
                        <div class="px-2">
                          <p class="text-base font-medium text-xlleading-none text-gray-700">
                            {{ product.name }}
                          </p>
                          <span class="text-xs text-gray-400">{{ product.created_at }}</span>
                        </div>
                      </td>
                      ...
                      <td class="py-2 items-center">
                        <router-link :to="`/admin/products/${product.uuid}/edit`" class="bg-cyan-900 hover:bg-cyan-800 text-white rounded text-sm py-3 px-5 rounded">
                          {{ $t('View') }}
                        </router-link>
                      </td>
                    </tr>
              ...
              <nav class="bg-gray-100 px-4 py-3 flex items-center justify-between sm:px-6">
                <div class="hidden sm:block">
                  <p class="text-sm leading-5 text-gray-700">
                    {{ $t('Per page') }}
                    <select @change="getProducts" class="border-none py-1 px-2 mr-10" v-model="perPage">
    ...
    <products-delete-model :show="productDeleteModel" @cancelled="productDeleteModel = false" @confirmed="deleteProductRows">
...
export default {
  ...
  methods: {
    deleteProductRows() {
      ...
      this.$axios
        .delete('http://192.168.1.186:8000/'+'api/v1/admin/food-items-rows-destroy/', this.selectedRows, {
          headers: {
    ...
    getProducts() {
      this.loading = true;
      this.$axios
        .get('http://192.168.1.186:8000/'+'api/v1/admin/food-items/', {
          params: {
            page: this.page,
            sort: this.sort,
