          ...
          <div class="bg-white py-2">
            <div class="bg-gray-100 p-2 mb-2 border sm:flex items-center justify-between">
              <div class="flex md:space-x-2">
                <router-link to="/admin/users/new" class="btn btn-app">
                  {{ $t('Add') }}
                </router-link>
              </div>
            </div>
            <div class="overflow-x-auto bg-white">
              ...
              <template v-if="userList.length">
                <table class="w-full whitespace-nowrap">
                  <tbody>
                    <tr v-for="(user, index) in userList" :key="index" tabindex="0" class="focus:outline-none h-16 border-b border-cyan-900 rounded hover:bg-gray-100">
                      <td class="pl-5 py-2 items-center">
                        <img :alt="$t('Avatar')" :src="user.avatar !== 'gravatar' ? user.avatar : user.gravatar" class="h-12 w-12 rounded-md" />
                      </td>
                      ...
                      <td class="py-2 items-center">
                        ...
                        <span class="text-xs text-gray-400">{{ $t('Role group') }}</span>
                      </td>
                      <td class="py-2 items-center">
                        <p class="text-base font-medium text-xlleading-none text-gray-700">
                      ...
                      <td class="py-2 items-center">
                        <router-link
                          :to="user.id !== $store.getters['app/getUserId'] ? `/admin/users/${user.id}/edit` : '/account'"
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      this.loading = true;
      this.$axios
        .get('http://192.168.1.186:8000/'+'api/v1/admin/users/', {
          params: {
            page: this.page,
            sort: this.sort,
