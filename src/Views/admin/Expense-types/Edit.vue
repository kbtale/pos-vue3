    ...
    <div class="my-6 max-w-1/6 mx-auto px-4 sm:px-6 lg:px-8">
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex min-w-0">
          <svg-vue class="content-title-icon" :icon="['fas', 'pen-solid']"></svg-vue>
...
export default {
  ...
  metaInfo() {
    return {
      title: this.$t('Edit expense type'),
    };
  },
  components: {
  ...
  methods: {
    updateExpenseType() {
      this.loading = true;
      this.$axios
        .put('http://localhost:8000/api/v1/admin/expense-types/'+this.$route.params.uuid, this.type, {
          headers: {
    ...
    getExpenseType() {
      this.loading = true;
      this.$axios
        .get('http://localhost:8000/api/v1/admin/expense-types/'+this.$route.params.uuid, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        })
    ...
    deleteExpenseType() {
      this.$axios
        .delete('http://localhost:8000/api/v1/admin/expense-types/'+this.$route.params.uuid, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        })
