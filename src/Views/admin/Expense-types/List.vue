<template>
  <main class="flex-1 relative overflow-y-auto focus:outline-none" tabindex="0">
    <div class="my-6 max-w-1/6 mx-auto px-4 sm:px-6 lg:px-8">
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex min-w-0">
          <svg-vue class="content-title-icon" :icon="['fas', 'sitemap']"></svg-vue>
          <h1 class="content-title">
            {{ $t('Expnese type') }}
          </h1>
        </div>
        <div class="flex md:mt-0 md:ml-4">
          <router-link class="btn btn-app mx-1" to="/admin/expense-types/new"> {{ $t('Create') }}</router-link>
          <button @click.prevent="$router.back()" type="button" class="btn btn-app mx-1">
            {{ $t('Back') }}
          </button>
        </div>
      </div>
    </div>
    <div class="my-6 max-w-1/6 mx-auto px-4 sm:px-6 lg:px-8">
      <div class="content-card-body">
        <!--<loading :active="loading"></loading>-->
        <template v-if="tableList.length > 0">
          <ul>
            <template v-for="(table, index) in tableList" :key="index">
              <li :class="{ 'border-t border-gray-200': index !== 0 }">
                <router-link
                  :to="'/admin/expense-types/' + table.id + '/edit'"
                  class="block hover:bg-gray-50 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
                >
                  <div class="flex items-center px-4 py-4 sm:px-6">
                    <div class="min-w-0 flex-1 flex items-center">
                      <div class="min-w-0 flex-1 pr-4 lg:grid lg:grid-cols-2 lg:gap-4">
                        <div class="flex">
                          <div class="inline-block align-middle text-sm font-medium leading-5 text-gray-900">
                            <div class="py-1 flex">
                              {{ table.title }}
                            </div>
                            <small class="text-xs text-gray-500">{{ $t('Created at') }} : {{ table.created_at }}</small>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div>
                      <svg-vue class="h-5 w-5 text-gray-400" :icon="['fas', 'angle-right']"></svg-vue>
                    </div>
                  </div>
                </router-link>
              </li>
            </template>
          </ul>
        </template>
        <template v-else="!loading">
          <no-content />
        </template>
      </div>
    </div>
  </main>
</template>
<script>
import NoContent from '../../../components/no-content.vue'
export default {
  name: 'List',
  components: {
    'no-content': NoContent,
  },
  metaInfo() {
    return {
      title: this.$t('Expnese type'),
    };
  },
  data() {
    return {
      loading: true,
      tableList: [],
    };
  },
  mounted() {
    this.getExpenseType();
  },
  methods: {
    getExpenseType() {
      this.loading = true;
      this.$axios
        .get('http://192.168.1.186:8000/'+'api/v1/admin/expense-types', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        })
        .then((response) => {
          this.tableList = response.data;
          this.loading = false;
        })
        .catch(() => {
          this.loading = false;
        });
    },
  },
};
</script>
