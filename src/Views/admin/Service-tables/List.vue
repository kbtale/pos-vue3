    ...
    <div class="my-6 max-w-1/6 mx-auto px-4 sm:px-6 lg:px-8">
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex min-w-0">
          <svg-vue class="content-title-icon" :icon="['fas', 'star']"></svg-vue>
          <h1 class="content-title">
            {{ $t('Service Tables') }}
          </h1>
        </div>
        <div class="flex md:mt-0 md:ml-4">
          <router-link class="btn btn-app mx-1" to="/admin/service-tables/new"> {{ $t('Create') }}</router-link>
          ...
          <ul>
            <template v-for="(table, index) in tableList" :key="index">
              <li :class="{ 'border-t border-gray-200': index !== 0 }">
                <router-link
                  :to="'/admin/service-tables/' + table.id + '/edit'"
                      ...
                      <div class="min-w-0 flex-1 pr-4 lg:grid lg:grid-cols-2 lg:gap-4">
                        <div class="flex">
                          <div class="inline-block align-middle text-sm font-medium leading-5 text-gray-900">
                            ...
                            <small class="text-xs text-gray-500">{{ $t('Created at') }} : {{ table.created_at }}</small>
                          </div>
                        </div>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    getTableList() {
      this.loading = true;
      this.$axios
        .get('http://192.168.1.186:8000/'+'api/v1/admin/service-tables/', {
          headers: {
