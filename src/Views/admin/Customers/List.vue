    ...
    <div class="my-6 max-w-1/6 mx-auto px-4 sm:px-6 lg:px-8">
      <div class="md:flex md:items-center md:justify-between">
        <div class="flex min-w-0">
          <svg-vue class="content-title-icon" :icon="['fas', 'users']"></svg-vue>
        ...
        <div class="p-4">
          <div class="w-full flex categories-center justify-between">
            <div tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl leading-normal text-gray-800">
              <input type="text" id="search" v-model="filters.search" :placeholder="$t('Search')" @change="getCustomers" class="form-input" />
            </div>
          ...
          <div class="bg-white py-2">
            <div class="bg-gray-100 p-2 mb-2 border sm:flex items-center justify-between">
              <div class="flex md:space-x-2">
                <router-link to="/admin/customers/new" class="btn btn-app">
                  {{ $t('Add') }}
                </router-link>
              </div>
            </div>
                  ...
                  <tbody>
                    ...
                    >
                      <td class="pl-5 py-2 items-center">
                        ...
                        <span class="text-xs text-gray-400">{{ customer.created_at }}</span>
                      </td>
                      ...
                      <td class="py-2 items-center">
                        <router-link :to="`/admin/customers/${customer.uuid}/edit`" class="bg-cyan-900 hover:bg-cyan-800 text-white rounded text-sm py-3 px-5 rounded">
                          {{ $t('View') }}
                        </router-link>
                      </td>
                    </tr>
              ...
              <nav class="bg-gray-100 px-4 py-3 flex items-center justify-between sm:px-6">
                <div class="hidden sm:block">
                  <p class="text-sm leading-5 text-gray-700">
                    {{ $t('Per page') }}
                    <select @change="getCustomers" class="border-none py-1 px-2 mr-10" v-model="perPage">
...
export default {
  ...
  methods: {
    ...
    getCustomers() {
      this.loading = true;
      this.$axios
        .get('http://192.168.1.186:8000/'+'api/v1/admin/customers', {
          params: {
            page: this.page,
            sort: this.sort,
