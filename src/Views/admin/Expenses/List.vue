<template>
  <main class="flex-1 relative overflow-y-auto focus:outline-none" tabindex="0">
        ...
        <div class="p-4">
          <div class="w-full flex item-center justify-between">
            <div tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl leading-normal text-gray-800">
              <input type="text" id="search" v-model="filters.search" :placeholder="$t('Search')" @change="getExpeses" class="form-input" />
            </div>
          ...
          <div class="bg-white py-2">
            <div class="bg-gray-100 p-2 mb-2 border sm:flex items-center justify-between">
              <div class="flex md:space-x-2">
                <template v-if="isThereSelectedRow">
                  <button class="btn btn-red" @click.prevent="expenseDeleteModel = true">{{ $t('Delete') }}</button>
                </template>
                <template v-else>
                  <router-link to="/admin/expenses/new" class="btn btn-app">
                    {{ $t('Add') }}
                  </router-link>
                </template>
              </div>
            ...
            <div class="overflow-x-auto bg-white">
              <!--<loading :active="loading"></loading>-->
              <template v-if="expensesList.length">
                    ...
                    >
                      ...
                      <td class="flex py-2 items-center">
                        <div class="px-2">
                          ...
                          <span class="text-xs text-gray-400">{{ expense.created_at }}</span>
                        </div>
                      </td>
                      ...
                      <td class="py-2 items-center">
                        <router-link :to="`/admin/expenses/${expense.id}/edit`" class="bg-cyan-900 hover:bg-cyan-800 text-white rounded text-sm py-3 px-5 rounded">
                          {{ $t('View') }}
                        </router-link>
                      </td>
                    </tr>
              ...
              <nav class="bg-gray-100 px-4 py-3 flex items-center justify-between sm:px-6">
                <div class="hidden sm:block">
                  <p class="text-sm leading-5 text-gray-700">
                    {{ $t('Per page') }}
                    <select @change="getExpeses" class="border-none py-1 px-2 mr-10" v-model="perPage">
    ...
    <expense-delete-model :show="expenseDeleteModel" @cancelled="expenseDeleteModel = false" @confirmed="deleteExpenseRows">
      <template #title>{{ $t('Delete selected rows') }}</template>
      {{ $t('The product under selected expenses will be removed as well') }}.
      {{ $t('Are you sure you want to delete the selected food expenses ?') }}
    </expense-delete-model>
  </main>
</template>
<script>
...
export default {
  ...
  methods: {
    deleteExpenseRows() {
      ...
      this.$axios
        .delete('http://192.168.1.186:8000/'+'api/v1/admin/expense-rows-destroy/', this.selectedRows, {
          headers: {
    ...
    getExpeses() {
      this.loading = true;
      this.$axios
        .get('http://192.168.1.186:8000/'+'api/v1/admin/expenses/', {
          params: {
            page: this.page,
            sort: this.sort,
        ...
        .then((response) => {
          this.expensesList = response.data.items;
