    ...
    <div class="my-1 max-w-1/6 mx-auto px-2">
      ...
      <template v-if="orders.length">
        <div :class="`py-2 text-cyan-900 grid grid-cols-1 md:grid-cols-${orders_in_a_row} gap-2 `">
          <div v-for="(order, index) in orders" :key="index" class="col-span-1 p-2 rounded-md mb-1 w-full shadow-md border border-cyan-900" :class="orders_bg">
            <div class="p-2">
            ...
            <select @change="updatePreparation(order)" aria-label="Sort by" class="form-input my-1" id="chef" v-model="order.chef_id">
              ...
              <template v-for="(chef, index) in chefList" :key="index">
                <option :value="chef.id">{{ chef.name }} - {{ chef.email }}</option>
              </template>
            </select>
...
export default {
  ...
  methods: {
    ...
    decodeOrderType(orderkey) {
      ...
      } else console.log("Order types not found")
