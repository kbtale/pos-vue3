                  ...
                  <div class="ml-2 flex-1 flex flex-col">
                    <div class="flex space-x-2 text-left text-xs font-medium text-gray-900">
                      ...
                      <div class="font-semibold text-gray-700 h-full border-gray-500 border-l w-36">
                        <p class="pl-2 product-topping">{{ item.price }} x {{ item.qty }}</p>
                        <p class="pl-2 pb-2 product-topping">{{ item.modifiers_price }}</p>
        ...
        <template v-if="!models.modifiersModel">
          ...
          <div class="grid grid-cols-3 gap-2 mt-2">
            <div class="col-span-3">
              ...
              <input :placeholder="$t('Item name')" class="form-input mt-1" id="name" required type="text" v-model="editedCartItem.name" />
            </div>
            <div class="col-span-3 md:flex md:space-x-2">
              <div class="w-full">
                ...
                <input :placeholder="$t('Item price')" class="form-input mt-1" id="price" required type="number" v-model="editedCartItem.price" />
              </div>
              <div class="w-full">
                <label class="input-label" for="qty">
                  {{ $t('Quantity') }}
                </label>
                <input :placeholder="$t('Item qty')" class="form-input mt-1" id="qty" required type="number" v-model="editedCartItem.qty" />
              </div>
            </div>
          </div>
        </template>
        <div v-if="models.modifiersModel && editedCartItem.modifiers" class="col-span-3">
          <strong class="text-gray-600">{{ $t('Update food modifiers') }}</strong>
          <div class="min-h-36 md:h-80 overflow-y-auto p-4">
            <div v-for="(modifier, key) in editedCartItem.modifiers" class="p-4 bg-gray-50 mb-1 border border-gray-200 rounded-md flex space-x-2">
              ...
              <div v-if="modifier.is_checked" class="flex justify-between items-center">
                ...
                <input disabled class="w-10 border-none px-1 pt-1 text-center" v-model="computedQty" />
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
              ...
              <div class="col-span-3 md:flex md:space-x-2 mt-5">
                <div class="w-20">
                  ...
                  <div class="pt-1">
                    <input aria-label="checkbox" type="checkbox" id="partner" v-model="currentCustomer.partner" class="form-checkbox border cursor-pointer h-5 w-5 text-blue-600 transition duration-150 ease-in-out" />
                  </div>
                </div>
                <div class="w-full">
                  <label class="form-label" for="creditLimit">{{ $t('Credit Limit') }}</label>
                  <div class="pt-1">
                    <input aria-label="input" type="number" class="form-input" id="creditLimit" v-model="currentCustomer.creditLimit" :disabled="currentCustomer.partner != true" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="grid grid-cols-1 gap-2">
          ...
          <div class="col-span-3 mb-2">
            <div class="md:flex md:space-x-1">
              <div class="w-full border border-gray-300 py-1 px-2 mb-1 rounded">
                <p class="text-sm text-cyan-900">{{ $t('Subtotal') }}:</p>
                <span>{{ sale.cart_total_price }}</span>
              </div>
              <div class="w-full border border-gray-300 py-1 px-2 mb-1 rounded">
            ...
            <div class="flex space-x-1">
              ...
              <div class="w-full border border-gray-400 py-1 px-2 mb-1 rounded" :class="balenceAmount >= 0 ? 'bg-green-100' : 'bg-red-100'">
                ...
                <span>{{ balenceAmount }}</span>
              </div>
            </div>
          </div>
          <div class="col-span-3">
            <div class="grid grid-cols-4 md:grid-cols-5 gap-1">
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(7)">7</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(8)">8</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(9)">9</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(5)">+5</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(10)">+10</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(4)">4</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(5)">5</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(6)">6</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(50)">+50</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(100)">+100</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(1)">1</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(2)">2</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(3)">3</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(150)">+150</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(200)">+200</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="enterNumber(0)">0</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="sale.recipient_amount = totalAmount">
                {{ $t('Total') }}
              </button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="clearNumber(0)">x</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(1000)">1000</button>
              <button type="button" class="col-span-1 p-3 items-center text-center btn btn-app" @click.prevent="addNumber(5000)">5000</button>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.uuid) {
      ...
      this.getAlreadysubmittedOrder(this.sale);
    }
  ...
  computed: {
    ...
    totalAmount() {
      if (this.sale.tax.is_tax_included === 'true') {
  ...
  methods: {
    ...
    cartItemInfoEditUpdate() {
      this.updateCartItem(this.editedCartItem);
    ...
    saveOrder() {
      ...
      this.sale.signature = this.signatureDir;
      ...
      this.loading = true;
      console.log("The sale to store: " + JSON.stringify(this.sale))
    ...
    showSignatureModel(){
      this.models.signatureModel = true
    ...
    storeSignature() {
      ...
      this.axios.post('http://localhost:8000/api/v1/files/store-signature/', formData, {
        ...
        .catch(error => {
          console.log('Signature not loaded: ' + error)
        });
    }
  },
};
